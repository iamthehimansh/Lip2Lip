...
def generate_video_function(video_path, audio_path_, text_,quality="low"):
    ...
    if quality == "low":
        ...
        cmd+=f"python inference.py --checkpoint_path 'checkpoints/wav2lip_gan.pth' --face {video_path_fix} --audio '../output_synth_audio_final.wav' --pads {pad_top} {pad_bottom} {pad_left} {pad_right} --resize_factor {rescaleFactor} --nosmooth --outfile '../output_high_qual_hi.mp4'"+" && cd ../.."
        subprocess.run(cmd, shell=True,capture_output=True, text=True, timeout=50*60)
    else:
        cmd="cd content/video-retalking &&"

        video_path_fix = f"'../{resized_video_path}'"
        cmd+="rm -rf ../output_high_qual_hi.mp4 &&"
        cmd+=f"""python inference.py --face {video_path_fix} --audio {"../output_synth_audio_final.wav"} --outfile {'../output_high_qual_hi.mp4'}"""+ " && cd ../.."
